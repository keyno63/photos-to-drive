...
指定した転送先以下にあるリモートファイル同士を比較する場合は、次を実行します。写真ライブラリは読み取らず、Google Driveにも変更を加えません。
...
- `--execute`では、処理開始時に`--remote`以下の全ファイルを標準で再帰的に一覧化します。ローカル原本とサイズ・MD5が一致する既存ファイルがあれば、名前やサブフォルダが違っていても再アップロードせず、そのDriveパスを検証済みとして記録します。同じ実行内で先にアップロードした同一内容も再利用します。写真・動画とも同じ方法で判定します。EXIFや撮影日時だけでは同一性を保証できないため、判定には使いません。
- `--skip-remote-check`を指定すると、開始時のDrive再帰走査と重複照合を省略します。転送先が非常に大きく、重複がないと分かっている場合にだけ使用してください。
- Google OAuthの`drive.file`権限では、rcloneが作成したファイルだけが一覧に現れます。DriveのWeb画面や別アプリで追加したファイルも調べるには、そのremoteに一覧権限が必要です。MD5を返さない項目は同一判定の対象外となり、件数を開始時に表示します。
- 実行開始時に、検証済みファイル数、対象総数、残数、開始時刻を表示します。各ファイルの行には `[現在位置/総数]`、累計検証済み件数、経過時間を表示し、終了時にも総経過時間を表示します。再開時は以前の実行で検証済みのファイルも累計へ含めます。
- Ctrl+Cで停止可能。コピー途中は破棄、転送・検証の失敗では一時コピーを残します。次回起動時は専用stagingの残りを削除して元ファイルから再試行します。
- 新規アップロード先は、元の写真ライブラリのディレクトリ構成を使わないフラットな`remote/MD5-内部ファイル名`です。MD5を名前に含めるため、同じ内部ファイル名でも内容が違えば衝突しません。以前のバージョンで作成した`ライブラリ識別子/0〜F/`以下のファイルは移動しませんが、標準の重複チェックが再帰的に検出してそのまま再利用します。`rclone copyto --immutable`を利用し、既存の異なる内容を上書きしません。
