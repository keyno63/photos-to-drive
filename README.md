...
- New uploads use the flat destination `remote/MD5-internal-filename`, independent of the Photos library's source directory layout. Including the MD5 prevents collisions when the same internal filename has different content. Files previously uploaded below `library-identifier/0-F/` are not moved; the default recursive duplicate check finds and reuses them in place. `rclone copyto --immutable` prevents overwriting an existing object with different content.
